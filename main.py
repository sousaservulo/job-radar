...
def _enviar_digest_diario(perfil: Perfil):
    ...
    horario_atingido = agora.hour >= DIGEST_HORA_UTC

    if not horario_atingido:
        return
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Qual(is) perfil(is) rodar nesta execução. "
            "Opções: brasil, internacional, dev e suporte. "
            "É possível informar mais de um perfil."
        ),
    )
    parser.add_argument(
        "--once",
